...
export default function SeoServices() {
  ...
  const benefits = [
    { icon: '🔍', title: 'Higher Rankings', desc: 'Rank on page 1 for your most valuable keywords.' },
    { icon: '📈', title: 'Organic Traffic', desc: 'Sustainable traffic growth without paid advertising.' },
    { icon: '🎯', title: 'Targeted Audience', desc: 'Attract visitors who are actively looking for you.' },
    { icon: '💰', title: 'Better ROI', desc: 'SEO delivers the highest long-term return on investment.' },
    { icon: '🏆', title: 'Brand Authority', desc: 'Establish your brand as an industry leader online.' },
    { icon: '📊', title: 'Data-Driven', desc: 'Every decision backed by real analytics and insights.' },
  ]

  const services = [
    'Technical SEO Audit', 'Keyword Research & Strategy', 'On-Page Optimization',
    'Link Building Campaigns', 'Content Strategy', 'Local SEO',
    'E-commerce SEO', 'Monthly Reporting',
  ]

  const process = [
    { num: '01', title: 'SEO Audit', desc: 'Complete analysis of your current website performance.' },
    { num: '02', title: 'Strategy', desc: 'Custom SEO roadmap based on your goals and competitors.' },
    { num: '03', title: 'On-Page SEO', desc: 'Optimize every page for maximum search visibility.' },
    { num: '04', title: 'Content', desc: 'Create SEO-optimized content that ranks and converts.' },
    { num: '05', title: 'Link Building', desc: 'Earn high-quality backlinks from authoritative sites.' },
    { num: '06', title: 'Monitor', desc: 'Track rankings, traffic, and adjust strategy monthly.' },
  ]
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', width: '100%' }}>
          <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.8s ease' }}>
            ...
            <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(3rem, 7vw, 5.5rem)', fontWeight: 700, color: '#fff', lineHeight: 1.05, marginBottom: '24px' }}>
              SEO <span style={{ color: '#C9A24A' }}>Services</span>
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.60)', fontSize: '16px', lineHeight: 1.8, maxWidth: '560px', marginBottom: '40px' }}>
              We help businesses dominate search engines, attract high-quality organic traffic, and convert visitors into customers.
            </p>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              <Link href="/contact" style={{ padding: '14px 32px', background: '#C9A24A', color: '#0A0A0A', borderRadius: '6px', textDecoration: 'none', fontSize: '14px', fontWeight: 600 }}>Get Free SEO Audit →</Link>
              <Link href="/portfolio" style={{ padding: '14px 32px', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', borderRadius: '6px', textDecoration: 'none', fontSize: '14px' }}>View Results</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '100px 24px', background: '#111111', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '64px' }}>
            <p style={{ color: '#C9A24A', fontSize: '11px', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '14px', fontWeight: 500 }}>Why SEO Matters</p>
            <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#fff' }}>The Benefits of SEO</h2>
          </div>
      ...
      <section style={{ padding: '100px 24px', background: '#0A0A0A' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '64px', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#fff', marginBottom: '20px' }}>Full SEO Package</h2>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '14px', lineHeight: 1.8 }}>Our comprehensive SEO service covers every aspect needed to grow your organic presence.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {services.map(s => (
              <div key={s} style={{ padding: '16px 20px', background: '#111111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ color: '#C9A24A', fontSize: '14px' }}>✓</span>
                <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: '13px' }}>{s}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '100px 24px', background: '#111111', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 700, color: '#fff' }}>Our SEO Process</h2>
          </div>
      ...
      <section style={{ padding: '100px 24px', background: '#0A0A0A', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2.5rem, 5vw, 3.5rem)', fontWeight: 700, color: '#fff', marginBottom: '16px' }}>
            Ready to Dominate<br /><span style={{ color: '#C9A24A' }}>Google?</span>
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '15px', lineHeight: 1.7, marginBottom: '36px' }}>Get a free SEO audit and discover your growth opportunities.</p>
